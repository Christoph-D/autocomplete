...
suite("LlmClient", () => {
  ...
  test("omits temperature when the request does not specify one", async () => {
    let capturedInit: RequestInit | undefined;
    const fetchFn = (async (_url: unknown, init?: RequestInit) => {
      capturedInit = init;
      return {
    ...
    const client = createLlmClient({ fetch: fetchFn });
    const { temperature: _omit, ...reqWithoutTemp } = baseRequest();
    void _omit;
    await client.complete(reqWithoutTemp, new AbortController().signal);
    const body = JSON.parse(String(capturedInit?.body));
